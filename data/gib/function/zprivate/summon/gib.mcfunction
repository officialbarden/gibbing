...
$data modify entity @e[sort=nearest,limit=1,type=item_display,tag=gib.DISPLAY,tag=gib.NEW] transformation.scale set value [$(scale)f, $(scale)f, $(scale)f]
summon marker ~ ~ ~ {Tags:["gib.VECTOR", "gib", "gib.NEW"]}

$data modify storage gib:storage TEMP.VELOCITY set value $(velocity)
execute store result score @e[sort=nearest,limit=1,type=marker,tag=gib.VECTOR,tag=gib.NEW] gib.VELOCITY1000 run data get storage gib:storage TEMP.VELOCITY 1000
