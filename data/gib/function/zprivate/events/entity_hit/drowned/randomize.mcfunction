...
execute store result storage gib:storage MACRO.GIB.y float 0.01 run random value 0..100
...
execute store result storage gib:storage MACRO.GIB.scale float 0.01 run random value 10..30
$execute store result storage gib:storage MACRO.GIB.block int 1 run random value 0..$(len)

function gib:zprivate/events/entity_hit/drowned/block_choose with storage gib:storage MACRO.GIB
function gib:zprivate/events/entity_hit/drowned/macro with storage gib:storage MACRO.GIB
scoreboard players remove #GibCount gib.CONFIG 1
$execute if score #GibCount gib.CONFIG matches 1.. run return run function gib:zprivate/events/entity_hit/drowned/randomize {len: $(len)}
